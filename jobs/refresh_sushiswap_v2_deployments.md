# Job: Refresh SushiSwap V2 Factory Deployments

## Goal

Check for new SushiSwap V2 factory contract deployments that are not yet tracked in this project, and add them to `dim_dex_factory_addresses.sql`.

## Suggested Frequency

Monthly, or after any SushiSwap announcement of a new chain deployment.

## Source of Truth

SushiSwap's official V2 deployment registry:
https://github.com/sushiswap/v2-core

Cross-reference with:
- SushiSwap's GitHub org (https://github.com/sushiswap) for any new chain-specific deployment repos
...
**Important:** Uniswap V2, PancakeSwap V2, GammaSwap, and Camelot V2 share the same topic0 (all are Uniswap V2 forks). Do not confuse SushiSwap V2 factory addresses with those protocols. Only add addresses from the official SushiSwap V2 docs.
...
These entries in `dim_dex_factory_addresses.sql` power `stg_sushiswap_v2_pool_created`, which decodes `PairCreated` events from `evms.logs` for all tracked SushiSwap V2 factory addresses. Adding a new row directly to `dim_dex_factory_addresses.sql` means the staging model will automatically pick up historical and future pool creation events for that deployment on the next incremental run.
