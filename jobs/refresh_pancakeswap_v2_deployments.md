# Job: Refresh PancakeSwap V2 Factory Deployments

## Goal

Check for new PancakeSwap V2 factory contract deployments that are not yet tracked in this project, and add them to `dim_dex_factory_addresses.sql`.
...
PancakeSwap's official V2 deployment registry:
...
**Important:** Uniswap V2, SushiSwap V2, GammaSwap, and Camelot V2 share the same topic0 (all are Uniswap V2 forks). Do not confuse PancakeSwap V2 factory addresses with those protocols. Only add addresses from the official PancakeSwap V2 docs.
...
These entries in `dim_dex_factory_addresses.sql` power `stg_pancakeswap_v2_pool_created`, which decodes `PairCreated` events from `evms.logs` for all tracked PancakeSwap V2 factory addresses. Adding a new row directly to `dim_dex_factory_addresses.sql` means the staging model will automatically pick up historical and future pool creation events for that deployment on the next incremental run.
