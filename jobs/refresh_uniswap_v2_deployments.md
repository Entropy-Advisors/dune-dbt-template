# Job: Refresh Uniswap V2 Factory Deployments

## Goal

Check for new Uniswap V2 factory contract deployments that are not yet tracked in this project, and add them to `dim_dex_factory_addresses.sql`.
...
Uniswap's official V2 deployment registry:
https://docs.uniswap.org/contracts/v2/reference/smart-contracts/v2-deployments

Cross-reference with:
- Uniswap's GitHub org (https://github.com/Uniswap) for any new chain-specific deployment repos
...
**Important:** SushiSwap V2, PancakeSwap V2, GammaSwap, and Camelot V2 share the same topic0 (all are Uniswap V2 forks). Do not confuse Uniswap V2 factory addresses with those protocols. Only add addresses from the official Uniswap V2 docs.
...
These entries in `dim_dex_factory_addresses.sql` power `stg_uniswap_v2_pool_created`, which decodes `PairCreated` events from `evms.logs` for all tracked Uniswap V2 factory addresses. Adding a new row directly to `dim_dex_factory_addresses.sql` means the staging model will automatically pick up historical and future pool creation events for that deployment on the next incremental run.
